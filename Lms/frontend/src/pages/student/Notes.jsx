...
export default function Notes() {
  ...
  if (checkingFeature) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50/50 via-white to-orange-50/50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-amber-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-amber-700">Loading notes...</p>
        </div>
      </div>
    )
  }
  ...
  if (isLocked) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50/50 via-white to-orange-50/50 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-amber-200 p-8">
          <div className="text-center space-y-6">
            ...
            <div className="space-y-4">
              ...
              <Link
                to="/dashboard"
                className="block w-full py-3 border-2 border-amber-300 text-amber-700 font-semibold rounded-xl hover:bg-amber-50 transition-all"
              >
                Back to Dashboard
              </Link>
            </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50/50 via-white to-orange-50/50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <motion.div
            ...
            className="mb-8"
          >
            <h1 className="text-4xl md:text-5xl font-black text-amber-900 mb-3">
              Study Notes
            </h1>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[...Array(6)].map((_, index) => (
                <div key={index} className="h-48 bg-white rounded-2xl animate-pulse"></div>
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {classes.map((c, index) => (
                ...
                >
                  ...
                  >
                    <div className="h-full bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-amber-200 overflow-hidden">
                      <div className="p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-3 bg-white border border-amber-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none text-amber-900 placeholder-amber-500/60 w-full sm:w-64"
              />
            </div>
          </div>
        </div>
          ...
          <div className="space-y-8">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="space-y-4">
                <div className="h-8 w-48 bg-white rounded animate-pulse"></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {[...Array(3)].map((_, j) => (
                    <div key={j} className="h-64 bg-white rounded-2xl animate-pulse"></div>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {groupNotes.map((note) => (
                      <motion.div
                        ...
                        className="h-full"
                      >
                        <div className="h-full bg-white rounded-2xl shadow-lg border border-amber-200 overflow-hidden hover:shadow-xl transition-all duration-300">
                          <div className="p-6 h-full flex flex-col">
                            ...
                            <h3 className="text-lg font-bold mb-3 text-amber-900 line-clamp-2">
                              {note.title}
                            </h3>

                            <p className="text-sm text-amber-700 mb-4 flex-1 line-clamp-3">
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-amber-200">
                              ...
                              <div className="flex gap-2">
                                <Link
                                  to={`/student/notes/${classId}/view/${note._id || note.id}`}
                                  className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white rounded-xl font-semibold transition-all hover:shadow-lg"
                                >
            ...
            {groups.every(g => g.notes.filter(n => n.title.toLowerCase().includes(searchTerm.toLowerCase())).length === 0) && (
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="text-center py-12 bg-white rounded-2xl border-2 border-dashed border-amber-300"
              >

...
import { ShoppingCart, Star, GraduationCap, ArrowRight, Trash2, X, CheckCircle, BookOpen, Plus, Minus } from 'lucide-react'
...
export default function Books() {
  ...
  // Your backend URL (for Vercel deployment)
  ...
  const getImageUrl = (imagePath) => {
    ...
    // Check if it starts with a slash (relative path)
    if (imagePath.startsWith('/')) {
      return `${BACKEND_URL}${imagePath}`
    }
    
    // Otherwise, add slash and prefix with backend URL
    return `${BACKEND_URL}/${imagePath}`
  }
